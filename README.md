...
- **Minimum release age**: pnpm is configured to reject packages published less than 14 days ago (`minimum-release-age=20160` in `.npmrc`), guarding against supply chain attacks where malicious versions are typically detected and removed within hours. Override per-install with `pnpm install --minimum-release-age=0` when needed. To exempt specific packages or scopes (e.g. internal packages from a private registry), use `minimumReleaseAgeExclude` in `pnpm-workspace.yaml`:
  ```yaml
